...
@mcp.tool(
    name="edit_document",
    description="Edit a document by replacing the string in said document with new string"
)
def edit_document(
    doc_id: str = Field(description="Id of the document to edit"),
    old_str: str = Field(description="Text to replace, most be an exact match, including whitespace"),
...
def fetch_doc(doc_id: str) -> str:
    if doc_id not in docs:
        raise ValueError(f"The Document with id {doc_id} was not found")
    return docs[doc_id]
...
@mcp.prompt(
    name="format",
    description="Rewrite the content of the document selected into markdown format"
)
def format_document(
